...
// Комментарии
    ...
    Route::get('/progress/{chatId}/{messageId}', [FileDownloadController::class, 'progress'])
    ->name('telegram.download.progress');
    Route::get('/api/size/{chatId}/{messageId}', [FileDownloadController::class, 'getSize'])
    ->name('telegram.download.size');
Route::get('/download-sse/{chatId}/{messageId}', [FileDownloadController::class, 'downloadWithProgress'])
    ->name('telegram.download.sse');  
});
