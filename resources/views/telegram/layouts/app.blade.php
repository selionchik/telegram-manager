                ...
                <ul class="navbar-nav me-auto">
                    ...
                    {{-- <li class="nav-item">
                        <a class="nav-link {{ request()->routeIs('telegram.comments.unprocessed') ? 'active' : '' }}"
                            ...
                            <span class="badge bg-danger rounded-pill unprocessed-count" style="display: none;">0</span>
                        </a>
                    </li> --}}
                </ul>
                    ...
                    <select id="account-selector" class="form-select form-select-sm bg-white">
                        @foreach ($accounts ?? [] as $account)
                            <option value="{{ $account->name }}"
                                {{ session('current_account') == $account->name ? 'selected' : '' }}>
...
function updateUnreadCount() {
    $.ajax({
        ...
        error: function(xhr, status, error) {
            console.log('Не удалось получить количество комментариев', error);
            ...
            function changeProxy() {
                ...
                // Было: '/api/proxy/change'
                // Стало:
                $.ajax({
                    url: '{{ route('telegram.proxy.change') }}',
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
            ...
            $('#proxyInput').on('input', function() {
                ...
                // Отправляем на сервер для парсинга
                $.ajax({
                    ...
                    `);
                            ...
                            $('#proxyList').html(
                                '<div class="alert alert-warning">Прокси не найдены</div>');
                            $('#proxyPreview').show();
                        }
                    }
                });
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            // Обновляем список прокси в интерфейсе
                            updateProxyList();
                        } else {
            ...
            // Функция обновления списка прокси
            function updateProxyList() {
                $.get('/telegram/api/proxy/list', function(response) {
                    // Можно обновить какой-то виджет с прокси, если нужно
