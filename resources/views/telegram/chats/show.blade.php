                ...
                @foreach ($messages as $msg)
                    <div class="d-flex {{ $msg->out ? 'justify-content-end' : 'justify-content-start' }}"
                        ...
                        <div class="message-bubble p-3 rounded-3 {{ $msg->out ? 'bg-primary text-white' : 'bg-white' }}"
                            ...
                            @if ($msg->has_media)
                                <div class="mt-2 media-container" data-message-id="{{ $msg->id }}"
                                    ...
                                    @else
                                        ...
                                        <div class="image-placeholder bg-light rounded-3 d-flex flex-column align-items-center justify-content-center"
                                            ...
                                            onclick="downloadImage({{ $msg->id }}, this)">
                                ...
                                <div class="comments-section mt-3 border-top pt-2">
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <small class="text-muted">
                                            ...
                                            {{ $msg->replies_count }}
                                            {{ trans_choice('комментарий|комментария|комментариев', $msg->replies_count) }}
                                        </small>
                                    ...
                                    <div class="comments-list d-none" id="comments-{{ $msg->message_id }}">
                                        <!-- Комментарии подгрузятся через AJAX -->
    ...
    <script>
        $(function() {
            ...
            setInterval(function() {
                ...
                $.get('/api/chat/{{ $chat->id }}/messages?after=' + lastMsgId, function(messages) {
...
function downloadImage(messageId, element) {
    ...
    // Таймер для отслеживания времени ожидания
    ...
    const downloadUrl = '{{ route('telegram.download', ['message' => ':messageId']) }}'.replace(':messageId', messageId);
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data.success) {
                ...
                if (data.cached) {
                    // Файл уже был скачан - показываем сразу
                    if (data.type === 'video') {
                        container.html(`
